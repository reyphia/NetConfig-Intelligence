...
def parse_routeros_config(raw_text: str) -> ParseResult:
    ...
    for section, line in _split_sections(raw_text):
        try:
            ...
            if section.startswith("/system identity"):
                ...
                filter_acl.entries.append(
                    AccessControlEntry(
                        ...
                        source_address_list=kv.get("src-address-list"),
                        destination_address_list=kv.get("dst-address-list"),
                        in_interface_list=kv.get("in-interface-list"),
                        out_interface_list=kv.get("out-interface-list"),
            ...
            elif section == "/ip firewall address-list" and action == "add":
                name = kv.get("list")
                if name and name not in device.address_lists:
                    device.address_lists.append(name)

            elif section == "/interface list" and action == "add":
                name = kv.get("name")
                if name and name not in device.interface_lists:
                    device.interface_lists.append(name)
    ...
    device.ip_pools = list(pools.keys())
...
def _link_dhcp_pools(
    ...
    for server in dhcp_server_defs:
        ...
        dhcp_pool = DHCPPool(name=server_name, address_pool=pool_name)
