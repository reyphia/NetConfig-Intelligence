...
def analyze(raw_text: str, selected_vendor: str = "auto") -> AnalysisSession:
    ...
    findings = get_rule_engine().run_all(parsed.device, raw_text) + network_findings(parsed.device)
...
def update_interface(session: AnalysisSession, name: str, address: str, prefix_length: int) -> AnalysisSession:
    ...
    session.findings = get_rule_engine().run_all(session.device, session.rendered_text) + network_findings(session.device)
